...
import { checkout, createOrder, purchaseOrder } from '../actions';
import {
  getTotal,
  getCartProducts,
  getTotalProducts,
...
class IdentityContainer extends Component {
  ...
  handleSubmit = (values) => {
    const { customerId, purchaseOrder, totalProducts, quantityById } =
      this.props;
      ...
      purchaseOrder()
        ...
        .catch((err) => {
          throw new SubmissionError({
            _error: 'Please login before completing order...',
          });
        })
    );
  };
